...
# 每天0点更新用户 App 列表：激活虚拟环境、关闭旧进程、后台启动每日任务
set -e
set -o pipefail
...
REPO_DIR="$SCRIPT_DIR"
MAIN_SCRIPT="main.py"
LOG_FILE="$SCRIPT_DIR/run_update_app.log"
PID_FILE="$SCRIPT_DIR/run_update_app.pid"

# 时间戳
ts() { date +"%Y-%m-%d %H:%M:%S"; }

echo "[$(ts)] === 每日0点更新用户 App 列表 ==="
...
# 后台启动 每日0点更新任务
# 方式一（推荐）：使用我们新增的每日0点调度器
CMD="python -c 'from schedulers.app_jobs import run_update_apps_midnight_cron; run_update_apps_midnight_cron()'"

# 如需改为固定间隔运行（非对齐0点），可替换为：
# CMD="python main.py sync_apps_cron --interval-minutes 1440"
